...
class Exploradora extends Nau {
    ...
    public List<int[]> movimentsPossibles() {
        ...
        for (int i = 1; i < 8; i++) {
            if (fil + i < 8 && col + i < 8) moviments.add(new int[] {fil + i, col + i});
            if (fil + i < 8 && col - i >= 0) moviments.add(new int[] {fil + i, col - i});
            if (fil - i >= 0 && col + i < 8) moviments.add(new int[] {fil - i, col + i});
            if (fil - i >= 0 && col - i >= 0) moviments.add(new int[] {fil - i, col - i});
        }
        return moviments;
    }
}